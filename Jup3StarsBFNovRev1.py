...
to 1) centroid Jupiter 
   ...
   The code also has a bit od code so a jupiter image can be added to a non conjunction night data set  for testing 
...
def centroid_JupbyEdge(data, startx, starty):  # for later possible expansion to ellipse model  , angJup, sizejup):
    ...
    while (bDone <1):
        ...
        i=i+1
        if i>200: bDone=1    
    starty= int((pixN+pixS)/2)  
    startx= int((pixW+pixE)/2)  
    # 
...
file_series ="F:\\jupDef\\2023-10-26\\sao93016\\23_57_55\\sao93016_"
...
file_output = "3StarsNovBFrev1.csv" 
...
darkAve=1300     #true dark - 100 pedestal
...
##    image = image/flat_image
    ...
    useJupCorrect = 0       #  1- 2ang  2- 180  3- both
    if(useJupCorrect > 0):
 ...
 #                 
        ...
        for j in np.arange( 0 ,jBacsize,1):
            ...
            for i in np.arange( 0 ,jBacsize,1):
                ...
                #image[j][i]=imageJBacBox[j][i]-darkAve   # diagnostic shows the correction in the corner of the image
            
        if jj== -1 :   # -1 ignores print  
            hduj = fits.PrimaryHDU(np.uint16(image))
            hdulj = fits.HDUList([hduj])
            hdulj.writeto("C:\\jupiterd\\JupTest12-2_180_cor.fits",overwrite=True)
        ...
        if jj==0 :   # -1 ignores print  
            hduj = fits.PrimaryHDU(np.uint16(image))
            hdulj = fits.HDUList([hduj])
            hdulj.writeto("C:\\jupiterd\\JupTest12-2_180aftergradcor.fits",overwrite=True)
    else:   # needed for jupiter centroid print 
        JXfinal=JXEst
        JYfinal=JYEst

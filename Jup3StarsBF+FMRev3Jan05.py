# -*- coding: utf-8 -*-
"""
rev 3 Jan05-2023 by Francesco Meschia
Added distortion correction using up to 3rd-degree coefficients from Astrometrica 
...
CP = np.genfromtxt(parameter_file, delimiter=",",max_rows = 26,usecols=(0,1), missing_values=(0, 0))
...
CoefX3=float(CP[8][0])
CoefX4=float(CP[9][0])
CoefX5=float(CP[10][0])
CoefX6=float(CP[11][0])
CoefX7=float(CP[12][0])
CoefX8=float(CP[13][0])
CoefX9=float(CP[14][0])
CoefX10=float(CP[15][0])
CoefY1=float(CP[16][0])
CoefY2=float(CP[17][0])
CoefY3=float(CP[18][0])
CoefY4=float(CP[19][0])
CoefY5=float(CP[20][0])
CoefY6=float(CP[21][0])
CoefY7=float(CP[22][0])
CoefY8=float(CP[23][0])
CoefY9=float(CP[24][0])
CoefY10=float(CP[25][0])
...
##
    TempX = Centroid1X_final
    TempY = Centroid1Y_final
    TempX = TempX - XX/2
    TempY = TempY - YY/2
    DistCorX1 = CoefX1 + CoefX2 * TempX + CoefX3 * TempY +\
        CoefX4 * TempX**2 + CoefX5 * TempX * TempY + CoefX6 * TempY**2 +\
        CoefX7 * TempX**3 + CoefX8 * (TempX**2) * TempY + CoefX9 * TempX * TempY**2 + CoefX10 * TempY**3
    DistCorY1 = CoefY1 + CoefY2 * TempX + CoefY3 * TempY +\
        CoefY4 * TempX**2 + CoefY5 * TempX * TempY + CoefY6 * TempY**2 +\
        CoefY7 * TempX**3 + CoefY8 * (TempX**2) * TempY + CoefY9 * TempX * TempY**2 + CoefY10 * TempY**3
    TempX = Centroid2X_final
    TempY = Centroid2Y_final
    TempX = TempX - XX/2
    TempY = TempY - YY/2
    DistCorX2 = CoefX1 + CoefX2 * TempX + CoefX3 * TempY+\
        CoefX4 * TempX**2 + CoefX5 * TempX * TempY + CoefX6 * TempY**2+\
        CoefX7 * TempX**3 + CoefX8 * (TempX**2) * TempY + CoefX9 * TempX * TempY**2 + CoefX10 * TempY**3
    DistCorY2 = CoefY1 + CoefY2 * TempX + CoefY3 * TempY+\
        CoefY4 * TempX**2 + CoefY5 * TempX * TempY + CoefY6 * TempY**2+\
        CoefY7 * TempX**3 + CoefY8 * (TempX**2) * TempY + CoefY9 * TempX * TempY**2 + CoefY10 * TempY**3
    TempX = Centroid3X_final
    TempY = Centroid3Y_final
    TempX = TempX - XX/2
    TempY = TempY - YY/2
    DistCorX3 = CoefX1 + CoefX2 * TempX + CoefX3 * TempY+\
        CoefX4 * TempX**2 + CoefX5 * TempX * TempY + CoefX6 * TempY**2+\
        CoefX7 * TempX**3 + CoefX8 * (TempX**2) * TempY + CoefX9 * TempX * TempY**2 + CoefX10 * TempY**3
    DistCorY3 = CoefY1 + CoefY2 * TempX + CoefY3 * TempY+\
        CoefY4 * TempX**2 + CoefY5 * TempX * TempY + CoefY6 * TempY**2+\
        CoefY7 * TempX**3 + CoefY8 * (TempX**2) * TempY + CoefY9 * TempX * TempY**2 + CoefY10 * TempY**3
    Results.write("{:s},,{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},{:f},,{:f},{:f},{:f},{:f},{:f},{:f},{:f}\n"\
                  .format(hdul[0].header['DATE-AVG'],DistCorX1,DistCorY1,\
                   DistCorX2,DistCorY2,\
                   DistCorX3,DistCorY3,\
                   JXfinal,JYfinal,\

...
def centroid_JupbyEdge(data, startx, starty):  # for later possible expansion to ellipse model  , angJup, sizejup):
    ...
    estval= jupmodel(60)
    valYp= data[starty + 60][startx]
    valYn= data[starty - 60][startx]
    valXp= data[starty][startx + 60]
    valXn= data[starty][startx - 60]
    #ychange1= 2*(valYp-valYn)/(valYp+valYn)
    #xchange1= 2*(valXp-valXn)/(valXp+valXn)
    
    ychange= 2*(valYp-estval + estval-valYn)/estval    
    xchange= 2*(valXp-estval + estval-valXn)/estval
    #ychange1= 2*(valYp - valYn)/estval              #BF this does not give the correct result   as above   dont know why valYp-valYn= 56100 instead of -9436
    #xchange1= 2*(valXp - valXn)/estval
    a= valYp - valYn                     #BF a= 56100 is also wrong  should be -9436  when valYp= 8595 and valYn= 18031   
    newY=starty+ ychange
    newX=startx+ xchange
    xwork=startx + (int(xchange*10))/10
    ywork=starty + (int(ychange*10))/10
    #print("val yp,yn xp xn = ", valYp,valYn, valXp,valXn, "xchange, ychange =", xchange,ychange, "newX,newY=",newX,newY )
    
    startx=int(xwork)
    starty=int(ywork)
    #move  a little closer and redo with last estimate
    estval= jupmodel(53)
    valYp= data[starty + 53][startx]
    valYn= data[starty - 53][startx]
    valXp= data[starty][startx + 53]
    valXn= data[starty][startx - 53]
    #ychange1= 2*(valYp-valYn)/(valYp+valYn)
    #xchange1= 2*(valXp-valXn)/(valXp+valXn)
    ...
    besty=ywork+ ychange
    bestx=xwork+ xchange       
    #print("final step 2    at x,y= ",bestx,besty)
...
file_series ="F:\\jupDef\\2023-11-07\\sao93016\\21_27_04\\sao93016_"
...
file_output = "Oct20-ED127-ALL.csv" 
...
addJup=1
if(addJup>0):
    jupglow= "C:\\jupiterD\\jupglow\\11-7\\jup11-7-23_60-1s_crop.fit"
...
##    image = image/flat_image
    ...
    #hdulj = fits.HDUList([hduj])
    #hdulj.writeto("C:\\jupiterd\\JupTest2JupAdded.fits",overwrite=True)            
    ...
    useJupCorrect = 1
    if(useJupCorrect > 0):
        jupCX2, jupCY2 =centroid_JupbyEdge(image, jupGlowX,jupGlowY)   #for possible furture expansion to ellipse model of jupiter  add ,95,101)
        JXfinal=jupCX2
        JYfinal=jupCY2
        
        jupGlowX=jupCX2
        jupGlowY=jupCY2
        sepsq= (Y2Est-jupGlowY)**2 +  (X2Est-jupGlowX)**2 
        sep= sqrt(sepsq)
    
        theta= np.arctan2((jupGlowY-Y2Est)/sep,(jupGlowX-X2Est)/sep)   
        ...
        for j in np.arange( 0 ,jBacsize,1):
            ...
            for i in np.arange( 0 ,jBacsize,1):
                ...
                if(useJupCorrect==3): imageJBacBox[j][i]= (image[pixYcw][pixXcw] + image[pixYccw][pixXccw] +image[pixY180][pixX180])/3
                ...
                #image[pixY180][pixX180]=10000+10*j+i
 ...
 #
        ...
        for j in np.arange( 0 ,jBacsize,1):
            ...
            for i in np.arange( 0 ,jBacsize,1):
                ...
                image[newY][newX] = darkAve + image[newY][newX] - imageJBacBox[j][i]
                #image[j][i]=imageJBacBox[j][i]-1400   # diagnostic shows the correction in the corner of the image
            
    #if jj== 0 : 
        hduj = fits.PrimaryHDU(np.uint16(image))
        hdulj = fits.HDUList([hduj])
        hdulj.writeto("C:\\jupiterd\\JupTest11-21_corall3.fits",overwrite=True)
